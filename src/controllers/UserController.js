import { createUser, createUserWithPost, deleteUser, findUserbByEmail } from "../models/User.js";
...
export const getUserByEmail = async (req, res) => {
    try {
        const user =  await findUserbByEmail(req.params.email);
        res.status(201).json({ user });
    }
    catch (error) {
        res
        .status(500)
        .json({ error: "Failed to find user by email", message: error.message });
    }
};
